...
require('./config')
const { default: makeWASocket, BufferJSON, WA_DEFAULT_EPHEMERAL, generateWAMessageFromContent, downloadContentFromMessage, downloadHistory, proto, getMessage, MessageType, generateWAMessageContent, prepareWAMessageMedia } = require('@adiwajshing/baileys-md')
const fs = require('fs')
...
let isLinkks2 = args[0].match(/(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/)
 if (!isLinkks2) return m.reply(`mess.error.Iv`)
try {
  m.reply(mess.wait)
  ythd(args[0])
  .then((res) => {
...
sendMediaURL(from, dl_link,`nih kak`).catch(() => m.reply(`mess.error.api`))
 })		
  })
} catch (err) {
  m.reply(`eror`)
